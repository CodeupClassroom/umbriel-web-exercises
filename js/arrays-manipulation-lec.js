...
(function () {
    ...
    const myFavCheeses = cheeses.slice(cheeses.indexOf("Muenster"), cheeses.indexOf("Kraft American Singles") + 1);

    // By passing one argument, we start at the specified index and return an array to the end.
    const otherCheeses = cheeses.slice(7);
    ...
    const reversedCheeses = copyOfArray(cheeses);
    ...
    const alphaTeam = ["David", "Jay", "Javier", "Justin", "Dane", "Codey the Duck"];
    const goldTeam = ["Kenneth", "Laura", "Cody the Human", "Casey", "Ry", "Doc Rob"];
